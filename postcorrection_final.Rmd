...
predicted_probs_baces <- predict(baces_model, type = "response")
predicted_class_baces <- ifelse(predicted_probs_baces > 0.5, "Discontinue", "Continue")

# Create confusion matrix
confmat_baces <- table(Predicted = predicted_class_baces, 
                        Actual = baseline_data$treatment_status)
print(confmat_baces)

# accuracy
accuracy_baces <- sum(diag(confmat_baces)) / sum(confmat_baces)
print(paste("Baseline model accuracy:", round(accuracy_baces * 100, 2), "%"))

# ROC and AUC for baseline model
roc_baces <- roc(baseline_data$treatment_status, predicted_probs_baces, levels = c("Continue", "Discontinue"), direction = "<")
auc_baces <- auc(roc_baces)
print(paste("Baseline model AUC:", round(auc_baces, 4)))
```


```{r}
predicted_probs_full_wthreshold <- predict(baces_model, type = "response")
predicted_class_full_wthreshold <- ifelse(predicted_probs_full_wthreshold > 0.1, "Discontinue", "Continue")
...
```{r}
predicted_probs_full_wthreshold <- predict(baces_model, type = "response")
predicted_class_full_wthreshold <- ifelse(predicted_probs_full_wthreshold > 0.4, "Discontinue", "Continue")
