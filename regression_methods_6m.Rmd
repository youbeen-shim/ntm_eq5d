...
Primary Question:
Using 6 Month Follow-up variables, we want to predict CMQUIT_GROUP_2 (which is the treatment response group)

Secondary Question:
What is the contribution of baseline QSEQQALY in completing our primary goal?

NOTE:
Too many missing values. Could probably use the original values from "B", but still need to address how to fill in the gap.
(approx ~13 missing values exist at Baseline as of right now (check regression_methods.Rmd))
...
baseline_data <- left_join(baseline_data, updated_eq5d, by = "SUBJNO") %>%
  mutate(QSEQQALY = QSEQQALY_6M) # can change do Delta via setting SQSEQQALY = QSEQQALY_6M - QSEQQALY_B
...
ggplot(predicted_data_long, aes(x = QSEQQALY, y = probability, color = CMQUIT_GROUP_2)) +
  ...
  labs(title = "Predicted Probabilities by QSEQQALY Score",
       x = "QSEQQALY Score at 6-Month Followup",
