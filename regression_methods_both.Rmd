...
Primary Question:
Using delta & 6M QALY variables, we want to predict CMQUIT_GROUP_2 (which is the treatment response group)
...
qseqqaly_only_model <- multinom(CMQUIT_GROUP_2 ~ QSEQQALY_6M, 
                              data = baseline_data, 
                              trace = FALSE)
...
3-1. QSEQQALY-only (with delta added)
```{r}
# Create a model with QSEQQALY only
qseqqaly_delta_model <- multinom(CMQUIT_GROUP_2 ~ QSEQQALY_6M + QSEQQALY_delta, 
                              data = baseline_data, 
                              trace = FALSE)

# Calculate McFadden's pseudo R-squared
mcfadden_r2_qseqqaly <- 1 - logLik(qseqqaly_delta_model)/logLik(null_model)
print(paste("McFadden's pseudo R-squared (QSEQQALY & delta model):", round(as.numeric(mcfadden_r2_qseqqaly), 4)))

# Calculate accuracy
predicted_classes_qseqqaly <- predict(qseqqaly_only_model, type = "class")
accuracy_qseqqaly <- mean(predicted_classes_qseqqaly == baseline_data$CMQUIT_GROUP_2, na.rm = TRUE)
print(paste("QSEQQALY & delta model accuracy:", round(accuracy_qseqqaly * 100, 2), "%"))
```
...
# Plot
pred_prob <- ggplot(predicted_data_long, aes(x = QSEQQALY_6M, y = probability, color = CMQUIT_GROUP_2)) +
  geom_line(size = 1.2) +
  labs(title = "Predicted Probabilities by QALY 6M & Delta ",
       x = "QALY, at 6 Month Follow-up",
...
final_model <- multinom(CMQUIT_GROUP_2 ~ 
                          QSEQQALY_6M + QSEQQALY_delta + FTPFDLCO + RFCTFD + MHTNTX + AGE,
                        data = baseline_data, 
                        trace = FALSE)
